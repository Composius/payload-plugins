...
describe('ComposiusPayloadPluginPages', () => {
  ...
  test('the collection is registered with the SEO plugin without a second meta group', () => {
    ...
    // The plugin has to list `pages` with seoPlugin for the generate endpoints
    // to authorize the request, which also makes seoPlugin append its own
    // `meta` group on top of the one Pages builds. Exactly one must survive.
    const metaFields = pages.fields.filter((field) => (field as { name?: string }).name === 'meta')
  ...
  const generateTitle = async (config: Config, doc: Record<string, unknown>) => {
    ...
    const response = await endpoint!.handler({
      json: () => Promise.resolve({ collectionSlug: 'pages', doc }),
      payload: {
